...
pub fn view(state: &State) -> Element<'_, Message> {
    ...
    let level_info = text(format!("Level: {} | XP: {}/{}", state.level, state.xp, state.xp_for_next)).size(10);
    let mood = text(format!("Mood: {}", state.mood)).size(10);
    let muted = text(if state.muted { "🔇 Muted" } else { "🔊 Listening" }).size(10);
    let stats = column![
        text("Stats:").size(10),
    ...
    let back_btn = button(text("← Back").size(10))
        .on_press(Message::HideStatus);

    column![species, level_info, mood, muted, text("---").size(8), stats, text("---").size(8), back_btn]
        .spacing(4).align_x(Alignment::Center).width(Length::Shrink)
        .into()
}
...
fn stat_row(name: &str, value: u8) -> Element<'_, Message> {
    ...
    row![text(format!("{:<8}", name)).size(9), text(format!("{:>2}", value)).size(9), text(bar).size(9)]
        .spacing(2).into()
}
